...
var config = {
    ...
    statusPages: null,
    statusPageDefaultType: 'text/html'
}

config.wwwroot = process.env.NODE_HTTP_ROOT ? Number(process.env.NODE_HTTP_ROOT) : config.wwwroot;
...
if (!config.statusPages) {
    ...
    config.statusPages = Object.keys(http.STATUS_CODES).forEach(function (v) {
        config.statusPages[v] = null;
    });
}
...
http.createServer(async function (req, res) {
    ...
    try {
        ...
        try {
            if (!res.headersSent) {
                if (req.method == 'HEAD' || req.statusCode == 204 || !config.statusPages[res.statusCode]) {
                    res.writeHead(res.statusCode);
                } else {
                    await new Promise((resolve, reject) => {
                        var page = config.statusPages[res.statusCode];
                        
                        res.setHeader('Content-Type', config.mime(''));
                        res.setHeader('Content-Length', stats.size);
                        res.writeHead(res.statusCode);
                        
                        res.write(config.statusPages[res.statusCode], function (err) {
